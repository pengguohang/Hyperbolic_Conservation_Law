...
def save_fig(pred, u, folder_name, model_name):
    ...
    plt.imshow(pred, extent=[-1,1,0,2], aspect='auto',  cmap='coolwarm')
    plt.xlabel('x')
    plt.ylabel('t')
    ...
    plt.figure(figsize=(5,5))
    plt.imshow(np.abs(pred - u), extent=[-1, 1, 0, 2], aspect='auto',  cmap='coolwarm')
    plt.xlabel('x')
    plt.ylabel('t')
    ...
    # 绘制t时刻的折线图
    x = np.linspace(-1, 1, pred.shape[0])
    u_real = u[-60, :]
    u_pred = pred[-60, :] 
...
def main(args):
    ...
    model_name = args.model_name + '_' + str(epochs) + '_' + acti + '_' + str(range_value)
    label_path='./burgers_move_202.mat'
    X_SIZE = 202
    T_SIZE = 202
    ...
    # 均匀网格离散化：101*101
    # res: 所有散点坐标
    res, b_init, b_end, b_left, b_right = get_data([-1, 1], [2, 0], X_SIZE, T_SIZE) 
    res_test, _, _, _, _ = get_data([-1, 1], [2, 0], X_SIZE, T_SIZE) 
    ...
    # Train PINNs -- MLP
    model = PINNs(in_dim=2, hidden_dim=128, out_dim=1, num_layer=8, acti=acti).to(device)
    ...
    step_size = 5000
    min_loss = torch.inf
    saved_path = os.path.join('/home/data2/pengguohang/HCL/Burgers_move', model_name)
    ...
    #     elif args.model_name == 'MIX':
    #         # print('DFVM+PINNs')
    #         a = x_res <= t_res/2 + range_value
    #         b = x_res >= t_res/2 - range_value
    #         c = a & b
